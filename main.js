...
function livelyAudioListener(audioArray)  {
  ...
  if (audio.length > 2) {
    ...
    if(!starStarted)
    {
      starInterval = setInterval(anim, 16);
      starStarted = true;
    }
    ...
    audio.map((val, index, arr) => {
      ...
      // Rotate each bar
      ctx.rotate(2 * Math.PI * ratio + Math.PI); // Rotate 180 Degrees for base on bottom and higher freq on top
...
function livelyPropertyListener(name, val) {
  ...
  switch (name) {
    ...
    case "useMiddleImage":
      ...
      break;
    case "middleImage":
      middle.style.content = "url(" + val.replace('\\', '/') + ")";
      break;
    case "barWidth":
      barWidth = val;
      break;
